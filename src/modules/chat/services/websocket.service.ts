  ...
  async connect(roomName: string, token: string): Promise<void> {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.ws.onopen = () => {
          console.log(`WebSocket OPENED to room: ${roomName} at ${new Date().toISOString()}`);
          this.isConnecting = false;
        ...
        this.ws.onclose = (event) => {
          console.log(`WebSocket CLOSED for room: ${roomName}, Code: ${event.code}, Reason: ${event.reason}, Time: ${new Date().toISOString()}`);
          this.isConnecting = false;
      ...
      try {
        ...
        if (!this.isConnected()) {
          if (!this.currentRoom || !this.token) {
            reject(new Error('Cannot send message: No active connection and missing connection details'));
            return;
          }
          
          console.log('Connection lost, reconnecting before sending message...');
          try {
